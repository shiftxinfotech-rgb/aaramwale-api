...
export class AssetsService {
  ...
  async create(createAssetDto: CreateAssetDto): Promise<Asset> {
    ...
    if (existing) {
      throw new BadRequestException(
        `Asset with code "${assetCode}" already exists at outlet ${outletId}`,
      );
    }
  ...
  async update(id: number, updateAssetDto: UpdateAssetDto): Promise<Asset> {
    ...
    if (
      ...
      updateAssetDto.categoryId !== asset.categoryId
    ) {
      const category = await this.categoryRepository.findOne({
    ...
    if (updateAssetDto.outletId && updateAssetDto.outletId !== asset.outletId) {
      const outlet = await this.outletRepository.findOne({
        where: { id: updateAssetDto.outletId },
      });
      if (!outlet) {
        throw new NotFoundException(
    ...
    if (updateAssetDto.assetCode || updateAssetDto.outletId) {
      ...
      if (existing && existing.id !== id) {
        throw new BadRequestException(
          `Asset with code "${nextAssetCode}" already exists at outlet ${nextOutletId}`,
        );
      }
    }
