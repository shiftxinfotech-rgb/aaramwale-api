...
export class DashboardController {
  ...
  @ApiOperation({ summary: 'Get dashboard summary (revenue, sales, walk-ins, redemptions)' })
  @ApiQuery({ name: 'outletId', required: false, type: Number, description: 'Filter by outlet (ADMIN/SUPER_ADMIN only)' })
  @ApiResponse({ status: 200, description: 'Dashboard data', type: DashboardResponseDto })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async getDashboard(@Query('outletId') outletId?: string, @GetUser() user?: any) {
