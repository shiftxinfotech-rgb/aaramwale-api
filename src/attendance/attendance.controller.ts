...
export class AttendanceController {
    ...
    @ApiBody({
        schema: {
            type: 'object',
            properties: {
                outletId: { type: 'number', example: 1, description: 'Optional: defaults to user assigned outlet' }
            }
        }
    })
    ...
    @ApiResponse({ status: 200, description: 'Status retrieved successfully', type: AttendanceStatusResponseDto })
    ...
    @ApiResponse({ status: 200, description: 'History retrieved successfully', type: [AttendanceResponseDto] })
    async getHistory(@Req() req, @Query('days') days?: number) {
        const user = req.user as User;
        const data = await this.attendanceService.getHistory(user.id, days ? Number(days) : 7);
        return {
