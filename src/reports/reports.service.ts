...
export class ReportsService {
  ...
  async getCategoryWise(fromDate?: string, toDate?: string): Promise<any[]> {
    ...
    const passQuery = this.dataSource.createQueryRunner().manager.createQueryBuilder('pass_items', 'item')
    ...
    const wiQuery = this.walkInSessionRepository.createQueryBuilder('session')
      .select('session.categoryId', 'categoryId')
      .addSelect('COUNT(session.id)::int', 'totalWalkIns')
      .addSelect('COALESCE(SUM(session.finalAmount), 0)::float', 'totalRevenue');

    if (fromDate) {
      passQuery.andWhere('item.createdAt >= :fromDate', { fromDate: new Date(`${fromDate}T00:00:00.000Z`) });
      wiQuery.andWhere('session.sessionDate >= :fromDate', { fromDate });
    }
    if (toDate) {
      passQuery.andWhere('item.createdAt <= :toDate', { toDate: new Date(`${toDate}T23:59:59.999Z`) });
    ...
    for (const r of passRes) {
      passMap.set(Number(r.categoryId), { count: Number(r.totalPasses), rev: Number(r.totalRevenue) });
    }

    const wiMap = new Map<number, { count: number; rev: number }>();
    for (const r of wiRes) {
      wiMap.set(Number(r.categoryId), { count: Number(r.totalWalkIns), rev: Number(r.totalRevenue) });
    }
  ...
  async getAssetWise(fromDate?: string, toDate?: string): Promise<any[]> {
    const assets = await this.assetRepository.find({ where: { isActive: true } });
    ...
    const passMap = new Map<number, { count: number; rev: number }>();
    for (const r of passRes) {
      passMap.set(Number(r.assetId), { count: Number(r.totalPasses), rev: Number(r.totalRevenue) });
    }

    const wiMap = new Map<number, { count: number; rev: number }>();
    for (const r of wiRes) {
      wiMap.set(Number(r.assetId), { count: Number(r.totalWalkIns), rev: Number(r.totalRevenue) });
    }
  ...
  async getOutletWise(fromDate?: string, toDate?: string): Promise<any[]> {
    const outlets = await this.outletRepository.find({ where: { isActive: true } });

    // Query Passes finalAmount
    const passQuery = this.dataSource.createQueryRunner().manager.createQueryBuilder('passes', 'pass')
    ...
    if (fromDate) {
      passQuery.andWhere('pass.createdAt >= :fromDate', { fromDate: new Date(`${fromDate}T00:00:00.000Z`) });
      wiQuery.andWhere('session.sessionDate >= :fromDate', { fromDate });
    }
    if (toDate) {
      passQuery.andWhere('pass.createdAt <= :toDate', { toDate: new Date(`${toDate}T23:59:59.999Z`) });
      wiQuery.andWhere('session.sessionDate <= :toDate', { toDate });
    }
    ...
    const passMap = new Map<number, { count: number; rev: number }>();
    for (const r of passRes) {
      passMap.set(Number(r.outletId), { count: Number(r.totalPasses), rev: Number(r.totalRevenue) });
    }

    const wiMap = new Map<number, { count: number; rev: number }>();
    for (const r of wiRes) {
      wiMap.set(Number(r.outletId), { count: Number(r.totalWalkIns), rev: Number(r.totalRevenue) });
    }
  ...
  async getTopCustomers(fromDate?: string, toDate?: string): Promise<any[]> {
    ...
    const passMap = new Map<number, { count: number; rev: number }>();
    for (const r of passRes) {
      passMap.set(Number(r.customerId), { count: Number(r.totalPasses), rev: Number(r.totalRevenue) });
    }

    const wiMap = new Map<number, { count: number; rev: number }>();
    for (const r of wiRes) {
      wiMap.set(Number(r.customerId), { count: Number(r.totalWalkIns), rev: Number(r.totalRevenue) });
    }
  ...
  async getMostUsedAssets(fromDate?: string, toDate?: string): Promise<any[]> {
    ...
    const query = this.dataSource.getRepository(Token).createQueryBuilder('token')
    ...
    if (fromDate) {
      query.andWhere('token.redeemedAt >= :fromDate', { fromDate: new Date(`${fromDate}T00:00:00.000Z`) });
    }
    if (toDate) {
      query.andWhere('token.redeemedAt <= :toDate', { toDate: new Date(`${toDate}T23:59:59.999Z`) });
    }
  ...
  async getEmployeeWise(fromDate?: string, toDate?: string): Promise<any[]> {
    ...
    const passRes = await passQuery.groupBy('pass.generatedByUserId').getRawMany();
  ...
  async getConsumptionStats(fromDate?: string, toDate?: string): Promise<any> {
    ...
    const query = this.dataSource.getRepository(Token).createQueryBuilder('token')
      ...
      .addSelect('COALESCE(SUM(CASE WHEN token.isFreeConsumption = true THEN token.redeemedQuantity ELSE 0 END), 0)::int', 'freeConsumed');
    ...
    return {
      ...
      paidSessionsConsumed: Number(res.totalConsumed) - Number(res.freeConsumed),
    };
  }

  async getEmployeeRedemptions(fromDate?: string, toDate?: string): Promise<any[]> {
  ...
  async getCustomerUsageHistory(customerId: number): Promise<any[]> {
    ...
    return this.dataSource.getRepository(Token).createQueryBuilder('token')
